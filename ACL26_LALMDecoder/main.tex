...
and one each of Swahili, Tamil and Urdu --- evaluated across the baseline models. The trained checkpoints are being evaluated on the
same sets; that sweep is still running, and we report it as coverage allows.
...
Training the projector at all is what matters. On every cell evaluated so far it beats the
strongest off-the-shelf model available for that language, by margins up to 52.8 WER where those
models produce degenerate output. And how much in-language audio the projector sees predicts how
