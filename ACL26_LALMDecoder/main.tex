...
off-the-shelf model we evaluate on the same test set, over the 33 cells evaluated so far.
Training wins on 13 of them, and which 13 is the result: the benefit tracks how badly the
off-the-shelf model was already doing (Spearman $\rho = -0.45$, $p = 0.009$). On the nine cells
whose baseline exceeds 50 WER the projector wins six, with a median gain of 25.0 WER and a
largest of 77.1; on the ten whose baseline is under 20 WER it wins one, and costs a median of
5.0 WER.
...
% TODO: the sweep is at 33 of 43 cells. Re-run build_paper_tables.py before submission and
...
dialect-transfer axis: 20 held-out varieties in all --- 8 further Spanish, 7 English, 2 French,
and one each of Swahili, Tamil and Urdu --- evaluated across the baseline models and, on 13 of
them so far, the trained checkpoints.

The projector does not transfer across dialect. On its own training variety it beats the
baseline on 7 of 10 cells, with a median gain of 5.1 WER; on held-out varieties of the same
languages it beats the baseline on 2 of 13, and costs a median of 5.6 WER. The two it wins are
Tanzanian Swahili and Indian Urdu, both cases where the baseline was already above 45 WER --- so
they are the same weak-baseline effect as above rather than an exception to the dialect result.
A projector trained on one variety of a language is a system for that variety.
...
Training the projector at all is what matters, though not uniformly. Across 33 evaluated cells
it beats the strongest off-the-shelf model on 13, and the benefit tracks the baseline's weakness
($\rho = -0.45$, $p = 0.009$): up to 77.1 WER where those models produce degenerate output, and
